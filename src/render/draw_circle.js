...
function drawCircles(painter: Painter, sourceCache: SourceCache, layer: CircleStyleLayer, coords: Array<OverscaledTileID>) {
    ...
    for (let i = 0; i < coords.length; i++) {
        ...
        if (first || program.program !== prevProgram) {
         // const circle_radius = 0.0467974076256207389950340880397;
         //  const circle_radius  = painter.transform.scale /  layer.paint.get('circle-radius-pixel').value.value ;
            const circle_radius  = layer.paint.get('circle-radius-pixel').value.value/ (175/painter.transform.scale )  ;
            layer.paint._values['circle-radius'].value.value = circle_radius;
