...
const LngLat = require('./bj_lng_lat'),
    Point = require('@mapbox/point-geometry'),
    Coordinate = require('./coordinate'),
    util = require('../util/util'),
    interpolate = require('../style-spec/util/interpolate').number,
 ...
 *
 * 专用于北京地方系的切片数据 2018/7/20 by LLZ
 *
 * @private
 */
class Transform {
    ...
    constructor(minZoom: ?number, maxZoom: ?number, renderWorldCopies: boolean | void) {
        this.tileSize = 512; // constant
				...
				//北京地方坐标系，无法循环且没有必要重复的拼接全世界切片 by LLZ
        this._renderWorldCopies = false; //renderWorldCopies === undefined ? true : renderWorldCopies;
        this._minZoom = minZoom || 0;
        this._maxZoom = maxZoom || 22;

        this.latRange = [-600000.4975404218, 600000.4975404218];

        this.width = 0;
        this.height = 0;
        //中心点设在北京某地 by LLZ
        this._center = new LngLat(502945.7341003418,306151.6109161377);
        this.zoom = 0;
        this.angle = 0;
    ...
    get worldSize(): number {
    	/*表示某一级下全世界地图正方形边长的像素值 512 * 2^z
    	* 0级下，全世界有1个切片 512 * 2^0 = 512，边长为512px
    	* 1级下，全世界有2*2个切片 512 * 2^1 = 1024，边长为1024px
    	* 以此类推
    	* 2017/7/20 by LLZ
    	*/
    ...
    /**
     * Beijing54 Gauss mapX to absolute x coord
     * 北京地方坐标系mapX转像素坐标
     * @returns {number} pixel coordinate
     */
    lngX(lng: number) {
        return (9502136.49754042178 + lng) * this.worldSize / (2 * 10002136.4975404218);
    }
    /**
     * Beijing54 Gauss mapY to absolute y coord
     * 北京地方坐标系mapY转像素坐标
     * @returns {number} pixel coordinate
     */
    latY(lat: number) {
        ...
        return (10002136.4975404218 -lat) * this.worldSize / (2 * 10002136.4975404218);
    }

		/**
     * Absolute pixel x coord to Beijing54 Gauss mapX
     * 像素坐标转北京地方坐标系mapX
     * @returns {number} mapX coordinate
     **/
    xLng(x: number) {
        //return x * 360 / this.worldSize - 180;
        return x * (2 * 10002136.4975404218) / this.worldSize - 9502136.49754042178;
    }

    /**
     * Absolute pixel y coord to Beijing54 Gauss mapY
     * 像素坐标转北京地方坐标系mapY
     * @returns {number} mapY coordinate
     **/
    yLat(y: number) {
        //const y2 = 180 - y * 360 / this.worldSize;
        //return 360 / Math.PI * Math.atan(Math.exp(y2 * Math.PI / 180)) - 90;
        return 10002136.4975404218 - y * (2 * 10002136.4975404218) / this.worldSize ;
    }
    ...
    _calcMatrices() {
        ...
        //const verticalScale = this.worldSize / (2 * Math.PI * 6378137 * Math.abs(Math.cos(this.center.lat * (Math.PI / 180))));
				...
				const verticalScale = this.worldSize / (2 * 10002136.4975404218);//不太确定，需调试看看 2018/7/20 by LLZ
